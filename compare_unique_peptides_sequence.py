#!/usr/bin/env python3
"""
Step 8: Assess peptide sequence coverage and add valuable_sequence column.

Identifies parts of the full protein sequence with very little coverage (<2 single
AA overhangs). Peptides that cover such regions get valuable_sequence=1 (TRUE);
others get 0 (FALSE).
...
DEFAULT_INPUT = os.path.join(_SCRIPT_DIR, 'data', 'comet_frags_perc_openMS_confidence_accuracy_extraction_significance.csv')
...
def main():
    ...
    if args.output:
        ...
        out_csv = os.path.join(out_dir, f'{base}_valuable_sequences.csv')
    ...
    print(f"[Step 8] Low-coverage positions (<{args.min_overhangs} overhangs): {len(low_coverage_positions)}")
    print(f"[Step 8] Peptides with valuable_sequence=1: {n_valuable} / {len(df)}")

    df.to_csv(out_csv, index=False)
    print(f"[Step 8] Output saved to: {os.path.abspath(out_csv)}")
    ...
    if args.fasta and os.path.exists(args.fasta):
        ...
        if protein_sequence:
            try:
                ...
                # Build peptides list from CSV (one block per unique peptide)
                peptides = []
                seen = set()
                for idx in df.index:
                    row = df.loc[idx]
                    ...
                    peptides.append(p)
                if peptides:
                    unique_output = os.path.join(out_dir, f'{base}_unique_peptides.png')
                    create_accepted_peptides_protein_grid(
                        peptides, protein_sequence, pid or 'protein', len(protein_sequence),
                        unique_output, out_dir
                    )
                    print(f"[Step 8] Unique peptides plot saved to: {unique_output}")
