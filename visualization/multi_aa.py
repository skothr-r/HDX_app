...
# Use same overhang position logic as chromatogram plots (fragment pairs + protein positions, hyphen expansion, peptide-relative fallback)
try:
    from .chromatograms import get_single_aa_overhang_positions_from_row
except ImportError:
    get_single_aa_overhang_positions_from_row = None
...
def parse_single_aa_overhangs(csv_file, protein_id=None, fasta_file=None, return_peptide_indices=False):
    ...
    with open(csv_file, 'r') as f:
        ...
        for row_idx, line in enumerate(lines[data_start:], start=0):
            ...
            # BEST APPROACH: Use same overhang logic as chromatogram plots (fragment pairs + protein positions, hyphen expansion, peptide-relative fallback)
            if site_specific_str:
                # Get peptide_start from site_specific_residues
                site_specific_positions = []
                for item in site_specific_str.split(','):
                    item = item.strip()
                    if item and len(item) > 1:
                        m = re.match(r'^(\d+)([A-Z])$', item)
                        if m:
                            site_specific_positions.append(int(m.group(1)))
                peptide_start = min(site_specific_positions) if site_specific_positions else None
                clean_peptide = re.sub(r'\[.*?\]', '', peptide_seq) if peptide_seq else ''
                L = len(clean_peptide)
                has_single_aa = False
                if get_single_aa_overhang_positions_from_row and peptide_start is not None and L > 0:
                    row = {}
                    for col in ['single_aa_overhang_fragment_pairs', 'single_aa_overhangs_protein_positions', 'sequence_start_pos', 'sequence_positions']:
                        if col in header:
                            idx = header.index(col)
                            row[col] = row_data[idx] if idx < len(row_data) else None
                    peptide_positions = get_single_aa_overhang_positions_from_row(row, clean_peptide, None)
                    for p in peptide_positions:
                        if 1 <= p <= L:
                            protein_pos = peptide_start + p - 1
                            residue = clean_peptide[p - 1]
                            overhangs.append({
                                'start': protein_pos,
                                'end': protein_pos,
                                'residues': residue,
                            ...
                            has_single_aa = True
                if not has_single_aa:
                    # Fallback: index-aligned parsing (single_aa_list[i] with site_specific_list[i] and single_aa_pairs_list[i])
                        ...
                        if item and len(item) > 1:
                            ...
                            if match:
                                ...
                                if i < len(single_aa_pairs_list):
                                    fragment_pairs = [p.strip() for p in single_aa_pairs_list[i].split('|') if p.strip()]
                                overhangs.append({
                                    'start': protein_pos,
                                    'end': protein_pos,
                                    'residues': residue,
...
def create_peptide_rt_grid(csv_file, protein_id, fasta_file, output_file):
        ...
        for row_idx, row_data in enumerate(csv_reader):
            ...
            if site_specific_str:
                ...
                if protein_positions:
                    ...
                    # Sequence coverage positions: same logic as chromatogram plots (fragment pairs with hyphen
                    # expansion, single_aa_overhangs_protein_positions with peptide-relative fallback)
                    single_aa_positions = {}
                    single_aa_fragment_pairs = {}
                    clean_peptide = re.sub(r'\[.*?\]', '', peptide_seq) if peptide_seq else ''
                    L = len(clean_peptide)
                    if get_single_aa_overhang_positions_from_row and L > 0:
                        row = {}
                        for col in ['single_aa_overhang_fragment_pairs', 'single_aa_overhangs_protein_positions', 'sequence_start_pos', 'sequence_positions']:
                            if col in header:
                                idx = header.index(col)
                    ...
                    # Fallback: index-aligned parsing when chromatograms helper not available
                    if not single_aa_positions and single_aa_str and site_specific_list:
                        ...
                        for i, (protein_pos, protein_residue) in enumerate(site_specific_list):
                            if i < len(single_aa_list) and i < len(single_aa_pairs_list):
                                ...
                                for pair in pairs:
                                    normalized_pairs.add(normalize_fragment_pair(pair))
                                if normalized_pairs:
                                    single_aa_positions[protein_pos] = len(normalized_pairs)
    ...
    cbar = plt.colorbar(sm, ax=ax, orientation='horizontal', pad=0.22, shrink=0.8)
    ...
    # Adjust layout
    plt.tight_layout(rect=[0, 0.1, 1, 0.98], pad=1.2)
...
def get_filtered_peptide_indices(csv_file, protein_id, protein_length, fasta_file=None, top_n_limit=20, min_single_aa_overhangs=3):
    """
    Apply progressive coverage filtering logic:
    1. (Unique RT filter disabled - using all peptides)
    2. Include top N highest q-score peptides that result in single-AA or multi-AA overhangs, separately for c-ions and z-ions
    ...
    - Top N c-ion peptides per position (by q-score or sp_score)
    - Top N z-ion peptides per position (by q-score or sp_score)
    - Uses shorter overhangs first, then longer ones for uncovered positions
    - Only peptides with at least min_single_aa_overhangs single-AA overhangs are considered.
    ...
    with open(csv_file, 'r') as f:
        ...
        for row_idx, row_data in enumerate(csv_reader):
            ...
            # Require at least min_single_aa_overhangs single AA overhangs per peptide
            n_single_aa = len(single_aa_positions)
            if n_single_aa < min_single_aa_overhangs:
                continue
...
def create_block_constraint_map(overhangs, protein_sequence, protein_id, output_file):
    ...
    cbar = plt.colorbar(im, ax=ax, pad=0.12)
    ...
    plt.tight_layout(rect=[0, 0.1, 1, 0.98], pad=1.2)  # Leave more space at bottom for all labels
    plt.savefig(output_file, dpi=300, bbox_inches='tight', pad_inches=0.25)
