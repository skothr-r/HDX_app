#!/usr/bin/env python3
"""
Step 5: Filter Comet fragments by accuracy (5 ppm).
...
Diagnostics (in diagnostics/):
  1) accuracy_filter_observed_vs_expected_mz.png — scatter: theoretical m/z (x) vs observed/theoretical (y)
  2) accuracy_filter_delta_histogram.png — histogram of implied Δ (m/z) = obs − theo for excluded/rejected
...
def main():
    ap = argparse.ArgumentParser(description='Filter fragments by 5 ppm accuracy')
    ...
    if col_ions not in df.columns or col_mz not in df.columns:
        print("Error: CSV must have 'matched fragment ions' and 'matched fragment ion mz' columns.")
        sys.exit(1)
    ...
    if scatter_data:
        try:
            ...
            ax.set_title(f'Fragment accuracy: {n_frag_excluded} / {n_frag_total} fragments excluded (> {args.ppm} ppm); {len(peptides_rejected)} peptides rejected (0 sig fragments)', fontsize=11, color='0.9')
            ...
            diag_path = os.path.join(diagnostics_dir, 'accuracy_filter_observed_vs_expected_mz.png')
            ...
            if np.any(bad_mask):
                ...
                if len(delta_bad) > 0:
                    try:
                        ...
                        delta_path = os.path.join(diagnostics_dir, 'accuracy_filter_delta_histogram.png')
                        fig2.savefig(delta_path, dpi=150, bbox_inches='tight', facecolor='black')
                        plt.close(fig2)
                        print(f"Diagnostic delta histogram saved: {delta_path}")
    ...
    print(f"Output: {out_csv} ({n_pass} scans retained, {n_scans_dropped} scans discarded (0 fragments); {n_frag_excluded} fragments excluded by {args.ppm} ppm)")
